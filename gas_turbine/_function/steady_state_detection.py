...
def steady_division(power,interval = 20, alpha = 0.05):
    ...
    滑窗窗口interval为20，区间估计都显著性水平α为0.05
    Input: 
    ...
    for i in range(0, len(delta_power), interval):
        ...
        mu1,mu2 = mu - u * sigma / np.sqrt(interval),mu + u * sigma / np.sqrt(interval)#置信区间上下界
        if (mu1*mu2<0) :#包含0，说明处于稳态
    ...
    #保存模型
    joblib.dump(steady_ratio,'./model/steady_ratio.pkl')
...
def steady_training(delta_power_steady,delta_power_unsteady, number = 5):
    ...
    # 模型保存到本地路径
    joblib.dump(gmm,'E:/Python/turbine01/model/unsteady_model.pkl')
    joblib.dump((mean0,s_variance0),'E:/Python/turbine01/model/steady_model.pkl')
