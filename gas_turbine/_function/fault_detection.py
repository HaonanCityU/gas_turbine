...
def network_construction():
    ...
    # step1：定义模型结构，由前指向后，因——>果
    fault_model = BayesianNetwork([('CS', 'v'), ('CS', 's'), ('CS', 'f1'),
    ...
    # step2：添加概率，参考https://www.cnblogs.com/bonelee/p/14312448.html
    ...
    te_cpd = TabularCPD(variable='te', variable_card=2, evidence=['TF', 'TC', 'TD'], evidence_card=[2, 2, 2],
                        values=[[0.99, 0.1, 0.1, 0.0099, 0.1, 0.0099, 0.0099, 0.00099],
                                [0.01, 0.9, 0.9, 0.9901, 0.9, 0.9901, 0.9901, 0.99901]])  # 能效异常模式
...
def feature_extraction(data):
    feature = pd.DataFrame({'p2': [0], 't2': [0], 't4': [0], 'r': [0], 'm1': [0], 'm2': [0], 'ce': [0], 'te': [0],
                            'f1': [0], 'dp': [0], 'f2': [0], 'd': [0], 'fi': [0], 'v': [0], 's': [0], 'htb': [0],
                            'htw': [0]})
    try:
        if (data['p2'] > 0.5):
            feature['p2'] = [1]
